...
class UserCli:
    ...
    def help(file: TextIO) -> None:
        Private.help(UserCli.CMDS, file)
    ...
    def command(
        ...
        cmd_i = Private.command(UserCli.CMDS, input, file)
        if cmd_i is None:
            UserCli.help(file)
            return None
        else:
            args: list[str] = \
                Private.args(UserCli.CMDS[cmd_i], input, file)
            return ParsedCommand(UserCli.CMDS[cmd_i][0], args)
